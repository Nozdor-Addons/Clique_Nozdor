...
function Clique:SpellBookButtonPressed(frame, button)
    ...
    local name, rank
    if GameTooltip and (GameTooltip:IsOwned(parent) or GameTooltip:IsOwned(frame)) then
        local n, r = GameTooltip:GetSpell()
        if n then name, rank = n, r end
    end

    local spellId, link
    if type(SpellBook_GetSpellID) == "function" then
        spellId = SpellBook_GetSpellID(slot)
    end
    if spellId then
        if type(GetSpellLink) == "function" then
            link = GetSpellLink(spellId, bookType)
        end
        if not name or not rank then
            local sName, sRank = GetSpellName(spellId, bookType)
            name = name or sName
            rank = rank or sRank
        end
    end
