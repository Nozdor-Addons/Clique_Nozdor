...
function Clique:OptionsOnLoad()
    ...
    if parent and parent.IsEnabled and parent:IsEnabled() == 1 then
        SpellButton_OnEnter(parent)
        -- capture exact spell slot & book while cursor is on it
        if parent.GetID then self.__clique_slot = parent:GetID() end
        if SpellBookFrame and SpellBookFrame.bookType then self.__clique_book = SpellBookFrame.bookType end
        -- try capture name/rank from tooltip if available
        if GameTooltip and GameTooltip.GetSpell then
            local n, r = GameTooltip:GetSpell()
            if n then self.__clique_name = n; self.__clique_rank = r end
        end
    else
...
function Clique:FillListEntry(frame, idx)
    ...
    if tonumber(entry.arg2) then
        rankText = string.format("Rank %d", entry.arg2)
    ...
    local icon = entry.texture
    if entry.type == "spell" then
        if entry._spellID and GetSpellInfo then
            icon = select(3, GetSpellInfo(entry._spellID)) or icon
        end
        if (not icon) and entry.arg1 and GetSpellInfo then
            icon = select(3, GetSpellInfo(entry.arg1, entry.arg2)) or select(3, GetSpellInfo(entry.arg1)) or icon
        end
    end
    frame.icon:SetTexture(icon or "Interface\\Icons\\INV_Misc_QuestionMark")

    frame.binding:SetText((entry.modifier or "") .. self:GetButtonText(button))

    local arg1 = tostring(entry.arg1 or "")
    local arg2 = tostring(entry.arg2 or "")
    local arg3 = tostring(entry.arg3 or "")
    local arg4 = tostring(entry.arg4 or "")
    local arg5 = tostring(entry.arg5 or "")

    if entry.type == "action" then
        frame.name:SetText(string.format("Action Button %d%s", tonumber(arg1) or 0, entry.arg2 and (" on " .. arg2) or ""))
    elseif entry.type == "actionbar" then
        frame.name:SetText(string.format("Action Bar: %s", arg1))
    elseif entry.type == "pet" then
        local target = entry.arg2 and (" on " .. arg2) or ""
        frame.name:SetText(string.format("Pet Action %d%s", tonumber(arg1) or 0, target))
    elseif entry.type == "spell" then
        if entry.arg2 then
            frame.name:SetText(string.format("%s (%s)%s", arg1, rankText or "", entry.arg5 and (" on " .. arg5) or ""))
        else
    ...
    elseif entry.type == "target" then
        frame.name:SetText(string.format("Target Unit: %s", arg1 or ""))
    elseif entry.type == "focus" then
        frame.name:SetText(string.format("Set Focus Unit: %s", arg1 or ""))
    elseif entry.type == "assist" then
        frame.name:SetText(string.format("Assist Unit: %s", arg1 or ""))
    ...
    elseif entry.type == "macro" then
        frame.name:SetText(string.format("Macro: %s", arg1))
    else
        frame.name:SetText(string.format("%s", entry.type or ""))
    end
end
